...
from models.fsvae import SampledSpikeAct
#from models.fsvae import PSVAE_large
 #Load the trained model from the saved checkpoint
#vae_model =PSVAE_large()
#vae_model.load_state_dict(torch.load('best.pth'))
#vae_model.eval() 
class DDPMWrapper(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.conditional:
            ...
            with torch.no_grad():
                ...
                latent_x=self.vae.encode(x)#后验泊松分布的脉冲序列(N,128,T)
                raw=latent_x.mean(-1)#(N,128)
                 #u=torch.log(r_p)#(N,128)
                lambda_post=torch.exp(raw)# Convert to positive: lambda_post > 0
                prob_post = 1.0 - torch.exp(-lambda_post)
                prob_post = torch.clamp(prob_post, 0.0, 1.0)
                spike_post_t = SampledSpikeAct.apply(prob_post)
                x_recon = self.vae.decode(spike_post_t.unsqueeze(-1))
                cond = 2 * x_recon - 1
        ...
        eps_pred = self.online_network(
            x, eps, t, low_res=cond, z=z.squeeze() if self.z_cond else None#先是前向过程，x+eps+.....eps=x-t, 然后是(decoder(x_t, t, low_res=low_res, z=z))
        )
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        if self.eval_mode == "sample":
            ...
            #recons = self.vae(z)
            with torch.no_grad():
                recons = self.vae.sample(z.size(0))  # 里面会自动走TransformerPrior
            recons = 2 * recons - 1
            ...
            img = batch
            #recons = self.vae.forward_recons(img * 0.5 + 0.5)#将输入图像从 [-1, 1] 范围转换回 [0, 1] 范围，因为 VAE 模型通常期望输入图像的像素值在 [0, 1] 范围内。
            #recons = 2 * recons - 1
            latent_x=self.vae.encode(img)#后验泊松分布的脉冲序列(N,128,T)
            raw=latent_x.mean(-1)#(N,128)
            #u=torch.log(r_p)#(N,128)
            lambda_post=torch.exp(raw)# Convert to positive: lambda_post > 0
            prob_post = 1.0 - torch.exp(-lambda_post)
