...
def get_dataset(name, root, image_size, norm=True, flip=False, split="train", **kwargs):#norm是否对图像进行归一化（通常指是否转为 [-1,1]）
    assert isinstance(norm, bool)#确保传入的 norm 参数是布尔类型。
    ...
    if name == "celeba":
        dataset = CelebADataset(root, split="train",norm=norm, transform=transform, **kwargs)
...
def save_as_images(obj, file_name="output", denorm=True):
    ...
    for i, out in enumerate(obj_list):
        if out.ndim == 3 and out.shape[2] == 1:  # 灰度图 (H, W, 1)
            out = out[:, :, 0]  # 转为 (H, W)
        elif out.ndim == 3 and out.shape[2] == 3:  # RGB图 (H, W, 3)
            pass  # 保留原样
        elif out.ndim == 2:
            pass  # 已经是灰度图 (H, W)
        else:
